...
import sys
sys.path.append(".")
from src.vdp_func import vdp

#Setup test dataset
textbook_data = np.array([
...
df = pd.DataFrame(data=textbook_data, columns=['year', 'c', 'r', 'dpi', 'delta_dpi'])


#<https://theforce.hashnode.dev/variance-decomposition-proportions#heading-okay-so-i-have-collinearity-now-what> 
df = pd.concat([
    df['c'].iloc[:-1].reset_index(drop=True),
    df['r'].iloc[1:].reset_index(drop=True),
    df['dpi'].iloc[1:].reset_index(drop=True),
    df['delta_dpi'].iloc[1:].reset_index(drop=True)
], axis=1)

# Check pg.152 in Besley (1991) "Conditioning Diagonostics. Collinearity and Weak Data in Regression". Wiley. 
output = vdp(df, intercept=True)
print(output)
